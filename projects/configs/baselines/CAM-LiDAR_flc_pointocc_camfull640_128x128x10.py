...
# ============================================================
# FLC-PointOcc Fusion — Version B: cam preserved at 640
#
# Two-branch architecture:
#   Camera: img → ResNet50 → SECONDFPN → ViewTransformerLSSFlash
#           → cam_bev [B, 640, 128, 128]   (NO cam_adapter)
...
#   Fusion: cat → [B, 768, 128, 128] → fuse_conv 3x3 → [B, 256, 128, 128]
#           → CustomResNet2D (numC_input=256) → FPN_LSS → FLCOccHead
#
# A vs B: the only knob varied is whether cam_bev is compressed before
# fusion. Same lidar_adapter (1920→128), same fuse_out (256), identical
# post-fusion stack. If A ≈ B → cam compression is neutral; if A < B →
# cam carries information that 256-D adapter loses; if A > B → fuse_conv
# can't keep up with 768-D input at this width.
#
# LiDAR recipe identical to Version A (aligned with PointOcc-server).
# ============================================================
...
numC_Trans = 64
Dz = 10
cam_bev_channels = numC_Trans * Dz      # 640 = LSS output channels (kept as-is)

tpv_C = 192
lidar_bev_channels = tpv_C * Dz         # 1920

# Adapters: cam stays at 640, lidar 1920 → 128
lidar_adapter_out = 128
fuse_in_channels = cam_bev_channels + lidar_adapter_out   # 768
...
model = dict(
    ...
    lidar_proj_in=tpv_C,
    lidar_Dz=Dz,
    # No cam_adapter — cam_bev (640ch) feeds straight into cat.
    ...
    fuse_conv_cfg=dict(
        in_channels=fuse_in_channels,   # 768 = 640 + 128
