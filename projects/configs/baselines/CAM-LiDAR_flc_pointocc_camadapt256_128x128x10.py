...
# ============================================================
# FLC-PointOcc Fusion — Version A: cam compressed to 256
#
# Two-branch architecture:
#   Camera: img → ResNet50 → SECONDFPN → ViewTransformerLSSFlash
#           → cam_bev [B, 640, 128, 128]
#           → cam_adapter Conv1x1 → [B, 256, 128, 128]
#   LiDAR:  points → CylinderEncoder → TPVSwin → TPVFPN → TPVFuser
#           → lidar_3d [B, 192, 128, 128, 10]
#           → flatten Z first → lidar_bev [B, 1920, 128, 128]
#           → lidar_adapter Conv1x1 → [B, 128, 128, 128]
#   Fusion: cat → [B, 384, 128, 128] → fuse_conv 3x3 → [B, 256, 128, 128]
#           → CustomResNet2D (numC_input=256) → FPN_LSS → FLCOccHead
#
# LiDAR recipe is aligned with the verified server LiDAR-only PointOcc:
#   cyl_grid=[480,360,32], split=[8,8,8], tpv=cyl//2, Swin pretrained,
#   target eff bs=8 (2 GPU × samples_per_gpu=4), lr=2e-4, max_epochs=24.
#
# Fusion design follows BEVFusion (concat + Conv3x3, narrow post-fusion):
#   front-light/back-heavy — cam/lidar adapters small, post-fusion backbone
#   does the work. Compared to current 1x1+640: 3x3 has spatial smoothing
#   between modalities; 256 (vs 640) post-fusion matches BEVFusion.
# ============================================================
...
Dz = 10
cam_bev_channels = numC_Trans * Dz      # 640 = LSS output channels

# LiDAR TPV branch
tpv_C = 192                             # TPVFPN out channels per plane
lidar_bev_channels = tpv_C * Dz         # 1920 = flatten-Z input to lidar_adapter

# Adapters (front-light, BEVFusion-style)
cam_adapter_out = 256                   # cam: 640 → 256
lidar_adapter_out = 128                 # lidar: 1920 → 128
fuse_in_channels = cam_adapter_out + lidar_adapter_out   # 384
fuse_out_channels = 256                 # post-fusion narrow backbone input

# Post-fusion 2D encoder (back-heavy)
...
# ---- LiDAR-only PointOcc recipe (verified on server 4090×2) ----
cyl_grid_size = [480, 360, 32]
tpv_w = cyl_grid_size[0] // 2           # 240
...
model = dict(
    ...
    debug_camera_only_bypass=False,     # ← CRITICAL: real fusion path
    # ---- Camera branch (FLC step2 recipe) ----
    img_backbone=dict(
    ...
    img_view_transformer=dict(
        ...
        vp_megvii=False),
    # ---- LiDAR branch (PointOcc-server recipe) ----
    ...
    lidar_Dz=Dz,                        # 10
    # ---- Adapters (BEVFusion-style: small, before fuse) ----
    cam_adapter_cfg=dict(
        in_channels=cam_bev_channels,   # 640
        out_channels=cam_adapter_out,   # 256
        kernel_size=1, stride=1, padding=0, bias=False,
        norm_cfg=dict(type='BN'),
        act_cfg=dict(type='ReLU'),
    ),
    lidar_adapter_cfg=dict(
        in_channels=lidar_bev_channels, # 1920
        out_channels=lidar_adapter_out, # 128
        kernel_size=1, stride=1, padding=0, bias=False,
        norm_cfg=dict(type='BN'),
        act_cfg=dict(type='ReLU'),
    ),
    # ---- Fusion: BEVFusion-style Conv3x3 ----
    fuse_conv_cfg=dict(
        in_channels=fuse_in_channels,   # 384
        out_channels=fuse_out_channels, # 256
        kernel_size=3, stride=1, padding=1, bias=False,
        norm_cfg=dict(type='BN'),
        act_cfg=dict(type='ReLU'),
    ),
    fused_channels=fuse_out_channels,   # 256
    # ---- Cylindrical grid params (for voxels_coarse) ----
    ...
    occ_encoder_backbone=dict(
        type='CustomResNet2D',
        numC_input=fuse_out_channels,   # 256 ← was 640
        num_layer=[2, 2, 2],
...
# Target effective batch = 8 (matches PointOcc-server). On 2×4090: bs=4×2=8.
...
# lr=2e-4 matches PointOcc-server. Keep img_backbone lr_mult=0.1 since
